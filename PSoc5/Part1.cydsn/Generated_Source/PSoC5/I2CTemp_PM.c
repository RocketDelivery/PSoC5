...
I2CTemp_BACKUP_STRUCT I2CTemp_backup =
{
    I2CTemp_DISABLE,

#if (I2CTemp_FF_IMPLEMENTED)
    I2CTemp_DEFAULT_XCFG,
    I2CTemp_DEFAULT_CFG,
    I2CTemp_DEFAULT_ADDR,
    LO8(I2CTemp_DEFAULT_DIVIDE_FACTOR),
    HI8(I2CTemp_DEFAULT_DIVIDE_FACTOR),
#else  /* (I2CTemp_UDB_IMPLEMENTED) */
    I2CTemp_DEFAULT_CFG,
#endif /* (I2CTemp_FF_IMPLEMENTED) */

#if (I2CTemp_TIMEOUT_ENABLED)
    I2CTemp_DEFAULT_TMOUT_PERIOD,
    I2CTemp_DEFAULT_TMOUT_INTR_MASK,
#endif /* (I2CTemp_TIMEOUT_ENABLED) */
};

#if ((I2CTemp_FF_IMPLEMENTED) && (I2CTemp_WAKEUP_ENABLED))
    volatile uint8 I2CTemp_wakeupSource;
#endif /* ((I2CTemp_FF_IMPLEMENTED) && (I2CTemp_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: I2CTemp_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  I2CTemp_backup - The global variable used to save the component
...
{
#if (I2CTemp_FF_IMPLEMENTED)
    #if (I2CTemp_WAKEUP_ENABLED)
        uint8 intState;
    #endif /* (I2CTemp_WAKEUP_ENABLED) */

    /* Store registers before enter low power mode */
    I2CTemp_backup.cfg     = I2CTemp_CFG_REG;
    I2CTemp_backup.xcfg    = I2CTemp_XCFG_REG;
    I2CTemp_backup.addr    = I2CTemp_ADDR_REG;
    I2CTemp_backup.clkDiv1 = I2CTemp_CLKDIV1_REG;
    I2CTemp_backup.clkDiv2 = I2CTemp_CLKDIV2_REG;

#if (I2CTemp_WAKEUP_ENABLED)
    /* Disable master */
    I2CTemp_CFG_REG &= (uint8) ~I2CTemp_ENABLE_MASTER;

    /* Enable backup regulator to keep block powered in low power mode */
    intState = CyEnterCriticalSection();
    I2CTemp_PWRSYS_CR1_REG |= I2CTemp_PWRSYS_CR1_I2C_REG_BACKUP;
    ...
    */
    ...
    I2CTemp_XCFG_REG |= I2CTemp_XCFG_FORCE_NACK;
    while (0u == (I2CTemp_XCFG_REG & I2CTemp_XCFG_RDY_TO_SLEEP))
    {
    }

    /* Setup wakeup interrupt */
    I2CTemp_DisableInt();
    (void) CyIntSetVector(I2CTemp_ISR_NUMBER, &I2CTemp_WAKEUP_ISR);
    I2CTemp_wakeupSource = 0u;
    I2CTemp_EnableInt();
#endif /* (I2CTemp_WAKEUP_ENABLED) */

#else
    /* Store only address match bit */
    I2CTemp_backup.control = (I2CTemp_CFG_REG & I2CTemp_CTRL_ANY_ADDRESS_MASK);
#endif /* (I2CTemp_FF_IMPLEMENTED) */

#if (I2CTemp_TIMEOUT_ENABLED)
    I2CTemp_TimeoutSaveConfig();
#endif /* (I2CTemp_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2CTemp_Sleep
********************************************************************************
*
* Summary:
...
void I2CTemp_Sleep(void) 
{
    ...
    I2CTemp_backup.enableState = I2CTemp_DISABLE;
...
#else
    /* Store enable state */
    I2CTemp_backup.enableState = (uint8) I2CTemp_IS_ENABLED;

    if (0u != I2CTemp_backup.enableState)
    {
        I2CTemp_Stop();
    }
#endif /* (I2CTemp_WAKEUP_ENABLED) */
...
/*******************************************************************************
* Function Name: I2CTemp_RestoreConfig
********************************************************************************
*
* Summary:
*  The Enable wakeup from Sleep Mode selection influences this function
...
*
* Side Effects:
*  Calling this function before I2CTemp_SaveConfig() or
*  I2CTemp_Sleep() will lead to unpredictable results.
*
*******************************************************************************/
void I2CTemp_RestoreConfig(void) 
{
    ...
    if (I2CTemp_CHECK_PWRSYS_I2C_BACKUP)
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        /* Enable backup regulator in active mode */
        intState = CyEnterCriticalSection();
        I2CTemp_PWRSYS_CR1_REG &= (uint8) ~I2CTemp_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);

        /* Restore master */
        I2CTemp_CFG_REG = I2CTemp_backup.cfg;
    }
    else
    /* Low power mode was Hibernate - backup regulator is disabled. All registers are cleared */
    {
        ...
        intState = CyEnterCriticalSection();
        I2CTemp_ACT_PWRMGR_REG  &= (uint8) ~I2CTemp_ACT_PWR_EN;
        I2CTemp_STBY_PWRMGR_REG &= (uint8) ~I2CTemp_STBY_PWR_EN;
        CyExitCriticalSection(intState);

        /* Enable component in I2C_Wakeup() after register restore */
        I2CTemp_backup.enableState = I2CTemp_ENABLE;
    #endif /* (I2CTemp_WAKEUP_ENABLED) */

        /* Restore component registers after Hibernate */
        I2CTemp_XCFG_REG    = I2CTemp_backup.xcfg;
        I2CTemp_CFG_REG     = I2CTemp_backup.cfg;
        I2CTemp_ADDR_REG    = I2CTemp_backup.addr;
        I2CTemp_CLKDIV1_REG = I2CTemp_backup.clkDiv1;
        I2CTemp_CLKDIV2_REG = I2CTemp_backup.clkDiv2;
    }

#if (I2CTemp_WAKEUP_ENABLED)
    I2CTemp_DisableInt();
    (void) CyIntSetVector(I2CTemp_ISR_NUMBER, &I2CTemp_ISR);
    if (0u != I2CTemp_wakeupSource)
    {
        /* Generate interrupt to process incoming transaction */
        I2CTemp_SetPendingInt();
    }
    I2CTemp_EnableInt();
#endif /* (I2CTemp_WAKEUP_ENABLED) */

#else
    I2CTemp_CFG_REG = I2CTemp_backup.control;
#endif /* (I2CTemp_FF_IMPLEMENTED) */

#if (I2CTemp_TIMEOUT_ENABLED)
    I2CTemp_TimeoutRestoreConfig();
#endif /* (I2CTemp_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2CTemp_Wakeup
********************************************************************************
*
* Summary:
